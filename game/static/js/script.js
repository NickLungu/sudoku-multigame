(function(global) {
	...
	function getUnique(array, count) {
        ...
        const numericArray = tmp.map(item => {
		...
		var ret = [];
        // Make disabled indexes array

        var indexes_from_initial_state = [];

        if (typeof initialState !== 'undefined') {
            for (var i = 0; i < 9; i++) {
                for (var j = 0; j < 9; j++) {
                    if (typeof initialState['game']['matrix']['row'][i][j] === 'string') {
                        indexes_from_initial_state.push(i*9+j)
                    }
                }
            }
        }
		...
		for (var i = 0; i <= count; i++) {
            ...
            if (typeof initialState !== 'undefined') {
                var index = indexes_from_initial_state[i];
            } else {

                var index = Math.floor(Math.random() * tmp.length);
            }
			ret.push(numericArray[index]);
		}
		return ret;
	}
	...
	var Sudoku = function(container, settings) {
        ...
        if (typeof initialState !== 'undefined') {
	        ...
	        this.game.config = initialState["game"]["config"];

            this.game.matrix = initialState["game"]["matrix"];

            this.game.validation = initialState["game"]["validation"];

            this.game.values = initialState["game"]["values"];
        }
	};
	...
	Sudoku.prototype = {
		...
		start: function() {
			...
			var oneDArray = [].concat(rows[0],rows[1],rows[2],rows[3],rows[4],rows[5],rows[6],rows[7],rows[8]);
			...
			util.each(values, function(i, data) {
                // here we check that its object because sometimes number is less than difficulty.
			...
			if (typeof initialState === 'undefined') {
                ...
                // Send the serialized game state JSON to Django
                fetch('/save_game/', {
                    method: 'POST',
                    headers: {
                    ...
                    body: JSON.stringify({ game_state: gameStateJSON }),
                })
